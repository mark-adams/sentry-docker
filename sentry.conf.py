...
logger.addHandler(logging.FileHandler('/tmp/authlog'))
logger.setLevel(logging.DEBUG)

DEBUG = True
# BEGIN SENTRY DEFAULT CONFIG
...
# END SENTRY DEFAULT CONFIG

# BEGIN BITBUCKET CUSTOM CONFIG
import ldap

# ldap server settings
AUTH_LDAP_SERVER_URI = "ldaps://us-ad-cluster.office.atlassian.com:636"

# User DN for authentication
AUTH_LDAP_USER_DN_TEMPLATE = 'CN=%(user)s,OU=people,DC=office,DC=atlassian,DC=com'
AUTH_LDAP_BIND_AS_AUTHENTICATING_USER = True

# Restrict log in to only members that belong to staff group
from django_auth_ldap.config import LDAPSearch, NestedActiveDirectoryGroupType
AUTH_LDAP_GROUP_SEARCH = LDAPSearch(
    "OU=groups,DC=office,DC=atlassian,DC=com",
    ldap.SCOPE_SUBTREE,
    "(objectClass=group)"
)
AUTH_LDAP_GROUP_TYPE = NestedActiveDirectoryGroupType()
AUTH_LDAP_REQUIRE_GROUP = "cn=bitbucket-sentry-dl-users,ou=access,ou=groups,dc=office,dc=atlassian,dc=com"
...
AUTH_LDAP_USER_FLAGS_BY_GROUP = {
    "is_superuser": "cn=bitbucket-sentry-dl-administrators,ou=access,ou=groups,dc=office,dc=atlassian,dc=com"
}
